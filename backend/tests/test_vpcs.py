...
async def test_create_subnet(auth_client: AsyncClient):
    ...
    r = await auth_client.post(f"/api/vpcs/{vpc_id}/subnets", json={
        "name": "web-subnet", "cidr": "10.0.1.0/24", "gateway": "10.0.1.1",
    })
    assert r.status_code == 201
...
async def test_delete_subnet(auth_client: AsyncClient):
    ...
    vpc_id = vpc.json()["id"]
    subnet = await auth_client.post(f"/api/vpcs/{vpc_id}/subnets", json={
        "name": "temp", "cidr": "10.0.2.0/24",
    })
...
async def test_default_security_group_on_register(client: AsyncClient):
    ...
    login = await client.post("/api/auth/login", json={
        "username": "sguser", "password": "StrongP@ss1!",
    })
