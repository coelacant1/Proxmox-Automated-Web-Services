...
class SDNService:
    ...
    def get_zones(self, cluster_id: str | None = None) -> list[dict[str, Any]]:
        """Get all SDN zones."""
        try:
    ...
    def get_vnets(self, cluster_id: str | None = None) -> list[dict[str, Any]]:
        """Get all SDN VNets."""
        try:
    ...
    def get_paws_zone(self, cluster_id: str | None = None) -> dict[str, Any] | None:
        """Get the PAWS EVPN zone, if it exists."""
        zones = self.get_zones(cluster_id=cluster_id)
        for z in zones:
            if z.get("zone") == EVPN_ZONE:
                return z
        return None

    def get_vnet_status(self, vnet_name: str, cluster_id: str | None = None) -> dict[str, Any] | None:
        """Get a specific VNet's details from Proxmox."""
        try:
            return get_pve(cluster_id).get_sdn_vnet(vnet_name)
        except Exception as e:
    ...
    def create_vnet(
        ...
        try:
            get_pve(cluster_id).create_sdn_vnet(vnet_name, zone, tag=vxlan_tag, alias=alias)
            self._apply_sdn(cluster_id=cluster_id)
    ...
    def delete_vnet(self, vnet_name: str, cluster_id: str | None = None) -> None:
        """Delete a VNet and all its subnets."""
        try:
            subnets = self.get_subnets(vnet_name, cluster_id=cluster_id)
            for s in subnets:
                subnet_id = s.get("subnet") or s.get("cidr")
                if subnet_id:
                    try:
                        self.delete_subnet(vnet_name, subnet_id, cluster_id=cluster_id)
    ...
    def get_subnets(self, vnet_name: str, cluster_id: str | None = None) -> list[dict[str, Any]]:
        """List subnets on a VNet."""
        try:
            return get_pve(cluster_id).get_sdn_subnets(vnet_name)
        except Exception as e:
    ...
    def create_subnet(
        ...
        """
        try:
            get_pve(cluster_id).create_sdn_subnet(
                vnet_name,
                cidr,
                gateway,
                snat=snat,
    ...
    def delete_subnet(self, vnet_name: str, subnet_id: str, cluster_id: str | None = None) -> None:
        ...
        """
        try:
            get_pve(cluster_id).delete_sdn_subnet(vnet_name, subnet_id)
            self._apply_sdn(cluster_id=cluster_id)
    ...
    def allocate_vxlan_tag(self, used_db_tags: set[int] | None = None, cluster_id: str | None = None) -> int:
        """Allocate the next available VXLAN tag.

        Checks both Proxmox VNets and optionally the DB to avoid collisions.
        """
        vnets = self.get_vnets(cluster_id=cluster_id)
    ...
    def _apply_sdn(self, cluster_id: str | None = None) -> None:
        """Apply pending SDN changes on the cluster."""
        try:
            get_pve(cluster_id).apply_sdn()
        except Exception as e:
